...
public class ClosestPaitPoint {
	...
	public static void main(String[] args) {
		...
		System.out.println("The distance b/w them is"+distance(bruteforce[0], bruteforce[1]));
		System.out.println();
		System.out.println("By the divide and conquer emthid of O(nlog(n)), the closest points are:");
		System.out.println("Points:  x="+closestPair[0].x +"y= "+closestPair[0].y);
		System.out.println("Points:  x="+closestPair[1].x +"y= "+closestPair[1].y);
		System.out.println("The distance b/w them is "+distance(closestPair[0],closestPair[1]));
	}

	private static Points[] closestPair(Points[] a) {
		//create two copies of the original array
		//sort each array according to the x coordinate and y cordinates
	...
	private static Points[] closest(Points[] sortByX, Points[] sortByY) {
			if(sortByX.length <= 3)
				//System.out.println("Brute force for <= 3");
			...
			//if the closest pair p and q is a "split" pair, their positions in filtered data is at most 7 or 11 positions apart
			...
			System.out.println("Points:  x="+pair[0].x +"y= "+pair[0].y);
			System.out.println("Points:  x="+pair[1].x +"y= "+pair[1].y);
			System.out.println("*****");
				return pair;
	}

	private static Points[] bruteForce(Points[] sortByX, Points[] sortByY) {
			//brute force to find the closest pair whent he size is small enough
